...
class FlightSearchPage {
  ...
  constructor(page) {
    ...
    this.flightResults = page.locator('.result-section'); // Updated to match the potential actual class
  ...
  async searchFlights() {
    ...
    await this.page.waitForNavigation({ waitUntil: 'networkidle', timeout: 90000000 });
    await expect(this.page).toHaveURL(/.*\/vbook\/actions\/selectitinerary/);
  }

  async verifySearchResults() {
    await this.page.waitForSelector('.result-section', { timeout: 95000 });
    await expect(this.flightResults).toBeVisible();
  }

  async vui-si-cost-label(optionIndex = 0) {
  ...
  async continueToPassengerDetails() {
    ...
    await this.page.waitForNavigation({ waitUntil: 'networkidle', timeout: 900000 });
