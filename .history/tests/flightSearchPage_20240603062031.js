...
class FlightSearchPage {
  ...
  async searchFlights() {
    ...
    await this.page.waitForNavigation({ waitUntil: 'networkidle', timeout: 10000000000 });
    await expect(this.page).toHaveURL(/.*\/vbook\/actions\/selectitinerary/);
  }

  async verifySearchResults() {
    await this.page.waitForSelector('.result-section', { timeout: 9500000 });
    await expect(this.flightResults).toBeVisible();
  }

  async radio(optionIndex = 0) {
